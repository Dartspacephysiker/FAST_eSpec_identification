;;11/26/16
PRO JOURNAL__20161126__TEST_ROUTINE_FOR_OUTLIER_REMOVAL
  ...
  IF KEYWORD_SET(eSpec__remove_outliers) THEN BEGIN
        custom_title        = 'RemovedOutliers, '
        bonus_plotsuff      = 'removedOutliers'
     ENDIF ELSE BEGIN
        custom_title        = 'TestOutlierRemoval, '
        bonus_plotsuff      = 'testOutlierRemoval'
     ENDELSE

  custom_title += GET_TODAY_STRING(/DO_YYYYMMDD_FMT)
  ...
  ;; FOR k=0,1 DO BEGIN
  ;;    STR_ELEMENT,NEWELL__eSpec,structNames[k],INDEX=structInd
  ;;    IF structInd LT 0 THEN BEGIN
  ;;       PRINT,"Can't find " + structNames[k] + '! Skipping ...'
  ;;       CONTINUE
  ;;    ENDIF
        
  ;;    tmpOutlier_i = GET_OUTLIER_INDICES(NEWELL__eSpec.(structInd), $
  ;;                                       USER_INDS=user_inds, $
  ...
  ;;Already taken care of by GET_OUTLIER_INDICES
  ;; IF N_ELEMENTS(user_inds) GT 0 THEN BEGIN
  ;;    user_inds = CGSETINTERSECTION(user_inds,outlier_i)
  ;; ENDIF ELSE BEGIN
  ;;    user_inds = TEMPORARY(outlier_i)
  ;; ENDELSE
